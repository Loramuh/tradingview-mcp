...
Contains market overview, sector scanning, sector rotation, index analysis,
stock screening, trade plan generation, and Fibonacci retracement analysis
for Turkish equities. Mirrors the EGX service structure 1:1.
...
def get_bist_market_overview(timeframe: str = "1D", limit: int = 10) -> dict:
    ...
    screener = EXCHANGE_SCREENER.get("bist", _BIST_SCREENER)
        ...
        heatmap.append(
            {
                ...
                "sector_index": meta.get("sector_index", ""),
                "status": agg["status"],
...
def analyze_bist_index(index: str = "BIST30", timeframe: str = "1D", limit: int = 30) -> dict:
    ...
    Args:
        index:     Index name (BIST30, BIST50, BIST100, BISTKATILIM).
        timeframe: TradingView interval (default '1D').
    ...
    """
    from tradingview_mcp.core.data.bist_indices import BIST_INDICES, is_bist30_stock, is_bist50_stock
    ...
    if index_key not in BIST_INDICES:
        return {
            ...
            "usage": "Use BIST30, BIST50, BIST100, or BISTKATILIM",
        }
...
def screen_bist_stocks(
    ...
    for sym, ind, change in raw_results:
        try:
            ...
            # BIST minimum acceptable liquidity (lower than USD markets due to TRY denominations).
