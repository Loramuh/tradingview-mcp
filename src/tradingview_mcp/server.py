...
mcp = FastMCP(
    ...
    instructions=(
        ...
        "bist_market_overview, bist_index_analysis (BIST30/50/100), and more."
    ),
)
...
# ── BIST (Borsa Istanbul) market tools ─────────────────────────────────────────

@mcp.tool()
def bist_market_overview(timeframe: str = "1D", limit: int = 10) -> dict:
    """Comprehensive overview of the Borsa Istanbul (BIST) market.
...
def bist_sector_scan(sector: str = "", timeframe: str = "1D", limit: int = 20) -> dict:
    ...
    Args:
        sector: Sector name (banking, holding, automotive, energy_and_utilities,
                defense_aerospace, telecom, retail_and_food, reit, transportation,
                cement_glass, technology, mining, insurance, etc.).
                Leave empty to list all sectors.
        timeframe: One of 5m, 15m, 1h, 4h, 1D, 1W, 1M
        limit: Max results per sector (max 50)
    """
...
def bist_sector_scanner(
    ...
    Args:
        timeframe: One of 5m, 15m, 1h, 4h, 1D, 1W, 1M (default 1D)
        top_n_sectors: Number of top sectors to show stock picks for (1-20, default 5)
        top_n_stocks: Number of top stocks per highlighted sector (1-10, default 3)
        min_stock_score: Minimum stock score for picks (0-100, default 60)
    """
    timeframe = sanitize_timeframe(timeframe, "1D")
    top_n_sectors = max(1, min(20, top_n_sectors))
    top_n_stocks = max(1, min(10, top_n_stocks))
    min_stock_score = max(0, min(100, min_stock_score))
...
def bist_index_analysis(index: str = "BIST30", timeframe: str = "1D", limit: int = 30) -> dict:
    ...
    Uses a hybrid constituent source: tries TradingView screener for the live
    XU030/XU050/XU100 composition, falls back to a static baseline if the
    screener is unavailable.

    Args:
        index: BIST30, BIST50, or BIST100
        timeframe: One of 5m, 15m, 1h, 4h, 1D, 1W, 1M (default 1D)
        limit: Number of stocks to show in detail (max 100)
    """
    timeframe = sanitize_timeframe(timeframe, "1D")
...
def bist_stock_screener(
    ...
    Args:
        timeframe: One of 5m, 15m, 1h, 4h, 1D, 1W, 1M (default 1D)
        min_score: Minimum stock score to include (0-100, default 55)
        index_filter: Filter by index — BIST30, BIST50, BIST100 (empty = all BIST)
        limit: Number of results (max 50)
    """
    timeframe = sanitize_timeframe(timeframe, "1D")
    min_score = max(0, min(100, min_score))
